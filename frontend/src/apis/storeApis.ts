...
const StoreApi = {
  ...
  createStore: async (data: { seller_id: number; name: string; description?: string; image?: string; status?: string }) => {
    try {
      ...
      if (axios.isAxiosError(err)) {
        console.error('API createStore error:', err.response?.data || err.message);
      } else {
  ...
  updateStore: async (
    id: number | string,
    data: { name?: string; description?: string; image?: string; status?: string }
  ) => {
    try {
      const res = await axios.put(`${BASE_URL}/stores/${id}`, data, { headers: getAuthHeaders() });
      return res.data.store; // lấy từ key store
    } catch (err) {
      if (axios.isAxiosError(err)) {
        console.error('API updateStore error:', err.response?.data || err.message);
      } else {
  ...
  deleteStore: async (id: number | string) => {
    try {
      const res = await axios.delete(`${BASE_URL}/stores/${id}`, { headers: getAuthHeaders() });
      return res.data;
    } catch (err) {
      if (axios.isAxiosError(err)) {
        console.error('API deleteStore error:', err.response?.data || err.message);
      } else {

...
import { fetchAllStoreOrders, fetchAllStoreOrderItems } from "../../redux/features/store_order/store_orderSlice";
...
const StoreOrderPage: React.FC = () => {
  ...
  const { orders, loading, orderItems } = useSelector((state: RootState) => state.storeOrder);
  ...
  const handleViewDetails = async (order: any) => {
    ...
    const items = orderItems.filter((item: any) => item.store_order_id === order.store_order_id);
    // Gọi API lấy chi tiết từng order_item
    const token = localStorage.getItem("accessToken") || "";
    const promises = items.map((item: any) => OrderItemApi.getById(item.order_item_id, token));
  ...
  const handleConfirmShipping = async () => {
    ...
    try {
      await StoreOrderApi.updateStoreOrderStatus(confirmingOrder.store_order_id, "shipping");
      setConfirmModalOpen(false);
      setConfirmingOrder(null);
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "subtotal",
      render: (_: any, record: any) => (
        <span className="font-bold text-[#8b2e0f]">{formatCurrency(Number(record.subtotal))}</span>
      ),
    },
    {
      title: "Thanh toán",
      ...
      key: "payment_method",
      render: (v: string) => v === "zalopay" ? "Đã thanh toán" : "Thanh toán khi nhận hàng",
    },
    {
      title: "Địa chỉ",
      dataIndex: "address",
      ...
      render: (_: any, record: any) => (
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {record.status === "processing" && (
            <Button
              type="default"
              style={{ borderRadius: 0, borderColor: "#8b2e0f", color: "#8b2e0f" }}
              size="small"
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-8">
      ...
      />
      <Modal
        title={<span className="font-semibold text-lg">Chi tiết đơn hàng: {selectedOrder?.order_code}</span>}
        open={isModalOpen}
        ...
        {selectedOrder && (
          <div>
            <div className="grid grid-cols-2 gap-x-4 gap-y-2 mb-6">
              ...
              <div>
                <strong>Trạng thái:</strong> <Tag color={statusColor[selectedOrder.status] || "default"}>{statusLabel[selectedOrder.status] || selectedOrder.status}</Tag>
              </div>
              <div>
                <strong>Phương thức thanh toán:</strong> {selectedOrder.payment_method === "zalopay" ? "Zalopay" : "Tiền mặt (COD)"}
              </div>
              ...
              {detailedItems.map((item) => (
                <div key={item.order_item_id} className="flex items-center gap-4">
                  <div className="w-20 h-20 flex-shrink-0 bg-gray-100 border rounded-md flex items-center justify-center">
                    <img src={buildImageUrl(item.product?.image)} alt={item.product?.name || "Sản phẩm"} className="max-w-full max-h-full object-contain" />
                  </div>
            ...
            <div className="text-right mt-6 border-t pt-4">
              <h3 className="text-xl font-bold text-[#8b2e0f]">Tổng cộng: {formatCurrency(detailedItems.reduce((sum, item) => sum + item.price * item.quantity, 0))}</h3>
            </div>
          </div>
        )}
      </Modal>
      <Modal
        ...
        footer={null}
        centered
        title={<span className="font-semibold text-lg">Xác nhận giao hàng</span>}
      >
        <div className="mb-4">Bạn xác nhận đơn hàng <span className="font-semibold">{confirmingOrder?.order_code}</span> đã sẵn sàng giao cho đơn vị vận chuyển?</div>
        <div className="flex justify-end gap-2">
          <Button onClick={handleCloseConfirmModal} style={{ borderRadius: 0 }}>Huỷ</Button>
          <Button type="primary" style={{ backgroundColor: "#8b2e0f", borderColor: "#8b2e0f", borderRadius: 0 }} onClick={handleConfirmShipping}>
            Xác nhận
          </Button>
        </div>
      </Modal>

...
import { actFetchCartItems, type ICartItem } from "../../redux/features/cart/cartSlice";
...
const Cartpage: React.FC = () => {
  ...
  const rows = useMemo(() => {
    return items
      .map((it: ICartItem) => {
        const p = products.find((x: IProduct) => x.product_id === it.product_id);
        return p
          ? {
              key: `${it.cart_item_id}`,
              item: it,
  ...
  const sellers = useMemo(() => {
    const map = new Map<string | number, { id: string | number; name: string; avatar?: string | null }>();
    rows.forEach(({ product }) => {
      const anyProd: any = product as any;
      const id =
        anyProd.store_id ?? anyProd.seller_id ?? anyProd.sellerId ?? anyProd.store?.id ?? anyProd.seller?.id ?? `p-${product.product_id}`;
      const name = anyProd.store_name ?? anyProd.seller_name ?? anyProd.seller?.name ?? "Cửa hàng";
      const avatar = anyProd.store_avatar ?? anyProd.seller_avatar ?? anyProd.seller?.avatar ?? null;
  ...
  const groups = useMemo(() => {
    ...
    rows.forEach((r) => {
      ...
      const id =
        anyProd.store_id ?? anyProd.seller_id ?? anyProd.sellerId ?? anyProd.store?.id ?? anyProd.seller?.id ?? `p-${r.product.product_id}`;
      const name = anyProd.store_name ?? anyProd.seller_name ?? anyProd.seller?.name ?? "Cửa hàng";
      const avatar = anyProd.store_avatar ?? anyProd.seller_avatar ?? anyProd.seller?.avatar ?? null;
      if (!map.has(id)) {
    ...
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <main className="py-8">
        <div className="max-w-7xl mx-auto px-4">
          ...
          {error && (
            <div className="text-red-600 border border-red-300 p-3 mb-4">{error}</div>
          )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                {groups.map((g) => {
                  ...
                  return (
                    <section key={String(g.id)} className="border border-gray-200 bg-white">
                      {/* Shop header */}
                      <div className="flex items-center gap-3 p-4 border-b border-gray-200">
                        <div className="w-10 h-10 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center">
                          {avatarUrl ? (
                            <img src={avatarUrl} alt={g.name} className="w-full h-full object-cover" />
                          ) : (
                        ...
                        <h3 className="font-medium line-clamp-1 flex-1">{g.name}</h3>
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="text-sm text-gray-500 mt-1">Mã SP: {product.product_id}</div>
                              ...
                              <div className="w-28 text-right">
                                ...
                                <div className="mt-1 font-semibold">{formatPrice(subTotal)}</div>
                              </div>
                            </div>
                          );
                        })}
              ...
              <aside className="lg:sticky lg:top-4 border border-gray-200 p-4 h-max bg-white">
                ...
                <button
                  ...
                  title={selectedIds.length === 0 ? "Chọn sản phẩm để thanh toán" : "Tiến hành thanh toán"}
                >
